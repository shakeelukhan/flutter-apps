...
public class Game {
	...
	public Game() {
		...
		for (int i = 0; i < 35; i++) streakData[0][i] = streakData[1][i] = streakData[2][i] = 0;
		betStats = new int[4][5];
		for (int i = 0; i < 5; i++) betStats[0][i] = betStats[1][i] = betStats[2][i] = 0;
	}
	...
	public void prepare() {
		...
		for (int i = 0; i < 7; i++) gameData.get(gameIndex)[i] = 0;
		...
		for (int i = 0; i < 5; i ++) {
			...
			gameBets.get(gameIndex)[i][2] = gameBets.get(gameIndex)[i][3] = gameBets.get(gameIndex)[i][4] = gameBets.get(gameIndex)[i][5] = gameBets.get(gameIndex)[i][6] = betBankroll;
		}		
    ...
    //Read more: http://javarevisited.blogspot.com/2012/12/how-to-sort-hashmap-java-by-key-and-value.html#ixzz358HOUXhN
		...
		while (gameShoe.cardsLeft() >= min) {
			...
			if ( curr == 'T' ) {
				...
				for (int i = 1; i <= patternMaxLength; i++) {
					if ( (handIndexExclTie-i+1) >= 0 ) {
						patternExclTie = String.copyValueOf(gameDataExclTie.get(gameIndex), handIndexExclTie-i+1, i);
			...
			if (curr != 'T') {
				for (int i = 0; i < 1; i++) {
					...
					if (gameBets.get(gameIndex)[i][0] != 'X') {
						if (gameBets.get(gameIndex)[i][0] == curr) {
							gameBets.get(gameIndex)[i][2] += betUnit;
							gameBets.get(gameIndex)[i][4] += (int)(betUnit*Math.pow(2,gameBets.get(gameIndex)[i][1]));
							gameBets.get(gameIndex)[i][1] = 0;
							gameBets.get(gameIndex)[i][7]++;
						} else {
							gameBets.get(gameIndex)[i][2] -= betUnit;
							gameBets.get(gameIndex)[i][4] -= (int)(betUnit*Math.pow(2,gameBets.get(gameIndex)[i][1]++));
						...
						gameBets.get(gameIndex)[i][3] = Math.min(gameBets.get(gameIndex)[i][2],gameBets.get(gameIndex)[i][3]);
						gameBets.get(gameIndex)[i][5] = Math.min(gameBets.get(gameIndex)[i][4],gameBets.get(gameIndex)[i][5]);
						gameBets.get(gameIndex)[i][6] = Math.max(gameBets.get(gameIndex)[i][1],gameBets.get(gameIndex)[i][6]);
						gameBets.get(gameIndex)[i][8] = Math.max(gameBets.get(gameIndex)[i][7],gameBets.get(gameIndex)[i][8]);
					}
				}
				
				// Follow winner
				gameBets.get(gameIndex)[0][0]=curr;
				// Follow loser
				if (curr == 'B') {
					gameBets.get(gameIndex)[1][0]='P';
				} else if (curr == 'P'){ 
					gameBets.get(gameIndex)[1][0]='B';
				}
				// Alternate if no tie
				if (gameBets.get(gameIndex)[2][0] == 'B') {
					gameBets.get(gameIndex)[2][0]='P';
				} else {
					gameBets.get(gameIndex)[2][0]='B';
				}
				// After consecutive banker/player wins
				if (bstreak == 2 || pstreak == 2) {
					if (curr == 'P') {
						gameBets.get(gameIndex)[3][0]='P';
						gameBets.get(gameIndex)[4][0]='B';
					} else {
						gameBets.get(gameIndex)[3][0]='B';
						gameBets.get(gameIndex)[4][0]='P';
					}
				} else {
					gameBets.get(gameIndex)[3][0]='X';
					gameBets.get(gameIndex)[4][0]='X';
...
/*
		...
		for (int i = 0; i < 5; i++) {
			...
			if (gameBets.get(gameIndex)[i][2] > gameBets.get(gameIndex)[maxProfit][2]) {
				maxProfit = i;
			}
			if (gameBets.get(gameIndex)[i][3] > gameBets.get(gameIndex)[leastCost][3]) {
				leastCost = i;
			}
			if (gameBets.get(gameIndex)[i][6] < gameBets.get(gameIndex)[losingStreak][6]) {
				losingStreak = i;
			}
			if (gameBets.get(gameIndex)[i][8] > gameBets.get(gameIndex)[winningStreak][8]) {
				winningStreak = i;
			}
		}
		betStats[0][maxProfit]++;
		betStats[1][leastCost]++;
		betStats[2][losingStreak]++;
		betStats[3][winningStreak]++;
		*/
		System.out.println("\n-> P=" + gameStats.get(gameIndex)[0] 
			+ " PMAX=" + gameStats.get(gameIndex)[1] 
			+ " B=" + gameStats.get(gameIndex)[2] 
			+ " BMAX=" + gameStats.get(gameIndex)[3] 
			+ " T=" + gameStats.get(gameIndex)[4]
			+ " FLIP=" + gameStats.get(gameIndex)[5] 
			+ " FLIPMAX=" + gameStats.get(gameIndex)[6] 
			+ " HANDS=" + handIndex); 
//			+ printStr); 
	}
...
/*		System.out.println("\n-> "
				+ " M.PROFIT=" + betStats[0][0] + "," + betStats[0][1] + "," + betStats[0][2] + "," + betStats[0][3] + "," + betStats[0][4]
				+ " L.COST=" + betStats[1][0] + "," + betStats[1][1] + "," + betStats[1][2] + "," + betStats[1][3] + "," + betStats[1][4] 
				+ " L.STREAK=" + betStats[2][0] + "," + betStats[2][1] + "," + betStats[2][2] + "," + betStats[2][3] + "," + betStats[2][4] 
				+ " W.STREAK=" + betStats[3][0] + "," + betStats[3][1] + "," + betStats[3][2] + "," + betStats[3][3] + "," + betStats[3][4]); 
	*/	
		int streakCount = 0;
		...
		for (int i = 1; i < 25; i++) {
			...
			if (streakCount != 0) System.out.print(" " + i + "=" + streakCount + "(" + df.format((float)100*streakCount/streakData[0][0]) + "%)");
		}
		System.out.print("\n B.STREAK:");
		for (int i = 1; i < 25; i++) {
			streakCount = streakData[1][i];
			if (streakCount != 0) System.out.print(" " + i + "=" + streakCount + "(" + df.format((double)100*streakCount/streakData[1][0]) + "%)");
		}
		System.out.print("\n FLIP.STREAK:");
		for (int i = 1; i < 25; i++) {
			streakCount = streakData[2][i];
			if (streakCount != 0) System.out.print(" " + i + "=" + streakCount + "(" + df.format((double)100*streakCount/streakData[2][0]) + "%)");
		}
		...
		for (int i = 0; i < patternMaxLength ; i++) {
			...
			while (patternsExclTie.hasNext()) {
				...
				if (patternExclTie.length() == (i+1) && ( patternExclTie.startsWith("P") || patternExclTie.startsWith("B") )) {
					printStr = " " + patternExclTie + "=" + patternDataExclTie.get(patternExclTie) + "(" + df.format((double)100*patternDataExclTie.get(patternExclTie)/patternDataExclTie.get(String.valueOf(i+1))) + "%)" + printStr;
				}
			}
